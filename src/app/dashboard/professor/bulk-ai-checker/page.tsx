...
export default function BulkAICheckerPage() {
  ...
  const handleCheckAIAndSimilarity = async () => {
    if (languageWarning) {
      setUserError("Please fix the language warning above.");
      return;
    }

    if (!process.env.NEXT_PUBLIC_SIMILARITY_DETECTOR_URL) {
      setUserError("AI service is not ready. Try again later.");
      return;
    }
          ...
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem className="hidden md:block">
                <BreadcrumbLink href="/dashboard/professor" className="text-teal-300 hover:text-teal-400">Home</BreadcrumbLink>
              </BreadcrumbItem>
